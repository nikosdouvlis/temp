...
import {
  ...
  Query,
  NotifyStrategy,
  BroadcastChannelNotifyStrategy,
} from './index';
...
const createEventBus = (opts?: EventBusOptions) => {
  return new EventBus<TestChannels, StandAloneEvents, StandAloneQueries>(opts);
};

describe.concurrent('eventBus', () => {
  beforeEach(() => {
    vi.useFakeTimers();
  });
  ...
  describe('error propagation with throwImmediately', () => {
    ...
    it('throws immediately if an error is thrown from inside a callback when throwImmediately is true', ({
      ...
      const sut = createEventBus({ throwImmediately: true });
      const mock1 = vi.fn(() => {
        throw new Error('mock1 error');
      });
      const mock2 = vi.fn();
  ...
  describe('notify strategies', () => {
    it('accepts and uses an injected notify strategy', async ({ expect }) => {
      ...
      const sut = createEventBus({ notifyStrategy });
    ...
    it('accepts and uses a broadcastChannel-based strategy', async ({ expect }) => {
      ...
      const sut = createEventBus({ notifyStrategy: strategy });
      sut.publish('scope:action', { payload: 'test' });
  ...
  describe('types', () => {
    it('converts a channel or eventBus to readonly', async () => {
      const _sut = createEventBus();
    ...
    it('converts a channel or eventBus to writeonly', async () => {
      const _sut = createEventBus();
